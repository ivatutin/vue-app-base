...
- **+** UX-полировка: sort-индикатор у несортированных колонок виден только при hover/focus заголовка (меньше шума); «мягкая» загрузка — skeleton только на первой загрузке (нет данных), при рефетче поверх данных — притушивание строк + тонкая прогресс-линия (`showSkeleton`/`showOverlay`); селектор размера страницы (`pageSizeOptions`, `table.setPageSize`).
