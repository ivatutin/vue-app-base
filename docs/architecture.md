...
#### RBAC на маршрутах, а не только в меню

Фильтрация пунктов в [sidebar-items.ts](../src/widgets/app-sidebar/model/sidebar-items.ts) — это UX, а не защита: она прячет ссылку, но прямой ввод URL и `router.push` из кода её обходят. Поэтому каждая страница, чей пункт меню скрывается по праву, обязана объявить то же право в `meta.permissions`.

Инвариант закреплён тестом [sidebar-permissions.test.ts](../src/widgets/app-sidebar/model/sidebar-permissions.test.ts): он проверяет, что у каждого защищённого пункта есть страница и что она требует то же право. Он же ловит пункты, ведущие в никуда.

#### Catch-all

[`src/pages/[...path].vue`](../src/pages/%5B...path%5D.vue) (`noAuth: true`) ловит неизвестные URL. Без него `matched` пустой, `to.meta.noAuth` — `undefined`, и guard считает маршрут защищённым: вместо «404» пользователь получал форму входа. Страница не редиректит, поэтому введённый адрес остаётся в строке — так ошибку видно и в логах, и при шаринге ссылки.
