...
export function formatTimeInterval (seconds: number, shortFormat = false) {
  ...
  const d = Math.floor((seconds % 31_536_000) / 86_400)
  const h = Math.floor(((seconds % 31_536_000) % 86_400) / 3600)
  const m = Math.floor((((seconds % 31_536_000) % 86_400) % 3600) / 60)
  const s = Math.floor((((seconds % 31_536_000) % 86_400) % 3600) % 60)
  return [
