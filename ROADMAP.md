...
### [P1] `meta.permissions` на защищённых маршрутах `done` (2026-07-21)
Ветка проверки прав в guard была, но `meta.permissions` не стоял ни на одной странице — RBAC держался на скрытых пунктах сайдбара, то есть на «защите сокрытием ссылки»: прямой ввод URL и `router.push` из кода обходили её целиком.

Сделано:
- Заведены страницы [`/users`](src/pages/users/ui/UsersPage.vue) (`user.read`) и [`/roles`](src/pages/roles/ui/RolesPage.vue) (`role.manage`) — пока заглушки, но с настоящей защитой маршрута. Заодно закрыты висячие ссылки сайдбара.
- Инвариант закреплён тестом [sidebar-permissions.test.ts](src/widgets/app-sidebar/model/sidebar-permissions.test.ts): каждый скрытый по праву пункт обязан иметь страницу, и страница обязана требовать то же право. Тест проверен на срабатывание — при удалении `meta.permissions` падает с объяснением.
- Конвенция описана в [how-to/add-page.md](docs/how-to/add-page.md) и добавлена в чек-лист PR.

Проверка структурная (читает исходник страницы): `definePage` — макрос, до сборки его meta не существует как значение, а поднимать в юнит-тесте полный роутер с генератором маршрутов дороже, чем польза.

### [P1] Catch-all маршрут `done` (2026-07-21)
Неизвестный URL давал пустой `matched`, из-за чего `to.meta.noAuth` был `undefined` — guard считал маршрут защищённым и уводил на login. Пользователь получал форму входа вместо «404», а уже залогиненный — необъяснимое поведение.

Добавлен [`src/pages/[...path].vue`](src/pages/%5B...path%5D.vue) с `noAuth: true`. Прежняя `src/pages/system/not-found/` удалена — на неё никто не ссылался, а catch-all не редиректит и сохраняет введённый адрес в строке, что для 404 правильнее (ошибка видна в логах и при шаринге ссылки).

### [P1] Статусы пользователя ≠ `active` `done` (2026-07-21)
`isAuthorized` требует `status === 'active'`, а guard сливал «не аутентифицирован» и «не допущен» в один редирект на login. Пользователь в `pending_verification` успешно логинился и тут же возвращался на форму, которая снова принимала те же данные — бесконечный цикл без единого слова о причине.

Сделано:
- Решение guard'а вынесено чистой функцией `resolveGuard` ([app/providers/resolve-guard.ts](src/app/providers/resolve-guard.ts)) — тестируется без поднятия роутера, `beforeEach` стал тонкой обёрткой.
- Три исхода вместо двух: нет сессии → `/auth/login`; статус ≠ `active` → `/system/account-status`; не хватает прав → `/system/forbidden`. Статус проверяется **раньше** прав, иначе заблокированному сообщали бы про права вместо настоящей причины.
- Новая страница [`/system/account-status`](src/pages/system/account-status/ui/AccountStatusPage.vue) с текстом под каждый статус, кнопкой «Проверить снова» (подтверждение происходит вне вкладки — в письме или SMS) и выходом. `noAuth: true` намеренно: страница обязана быть достижимой для того, кого guard не пускает.
- В `userStore` добавлен геттер `status`.

+10 тестов на `resolveGuard`, включая инвариант «не допущенного не отправляем на login» и приоритет статуса над правами.

Разблокирует Phase 1 auth-suite: после запуска регистрации `pending_verification` станет исходным статусом всех новых пользователей.

### [P1] Меню аккаунта: «Выйти» не работал `done` (2026-07-21)
Пункт был отрисован, но без обработчика — клик молча не делал ничего. Кнопка, которая выглядит рабочей и не работает, хуже отсутствующей: пользователь считает, что сломалось приложение.

Подключён к существующей странице `/auth/logout` (а не продублирован вызов `logoutFlow` в шапке — иначе два пути выхода разъедутся). «Мой профиль» помечен `disabled` до появления страницы.

Проверка в реальном браузере (Playwright + мок `/users/me`) вскрыла два дефекта, которых не было видно по коду:

- **`ListItem` терял `href` у RouterLink.** Компонент биндил `:href="tag === 'a' ? href : undefined"`, и на RouterLink уходил явный `href: undefined`; fallthrough-атрибуты применяются поверх собственных props корневого элемента, поэтому адрес, который RouterLink проставляет сам, стирался. Клик работал, поэтому баг жил незамеченным — но Ctrl+клик, «открыть в новой вкладке», «копировать адрес» и скринридер ломались. Затрагивал **все** пункты-ссылки в проекте, не только выход. Закрыт регрессионным тестом (проверен на срабатывание).
- **Кнопка меню аккаунта не имела доступного имени** — ни `title`, ни `aria-label`, только иконка. Скринридер объявлял «кнопка».

Попутно `LogoutPage` перестала глушить ошибку через `void`: `void` успокаивает линтер, но не рантайм — reject уходил в unhandledrejection и глобальный обработчик показывал снекбар с ошибкой поверх экрана «Вы вышли». Теперь ошибка sign-out логируется, а пользователю показывается успешный выход (локальная очистка гарантирована в `logoutFlow`).

### [P1] Шапка показывала меню аккаунта гостям `done` (2026-07-21)
Анонимный пользователь видит шапку на публичных страницах с default-layout — 404 и `/system/forbidden`. Там ему предлагались «Мой профиль» и «Выйти», хотя сессии нет.

Теперь гостю показывается кнопка «Войти» (ссылка на `/auth/login`), а меню аккаунта не рендерится вовсе. Условие — `isAuthenticated` (профиль загружен), а не `isAuthorized`: пользователю в `pending_verification` или `suspended` меню как раз нужно, чтобы выйти, и предлагать ему «Войти» было бы бессмыслицей.

Для этого `Button` научился рендериться ссылкой по пропу `to`. Это закрывает системный пробел: навигация, оформленная кнопкой, теряет `href` — а с ним Ctrl+клик, «открыть в новой вкладке» и адрес для скринридера. Атрибуты собираются по тегу, чтобы не повторить дефект `ListItem`, где `href` затирался явным `undefined`. +8 тестов.

Осталось на потом: «На главную» на страницах 404 и 403 всё ещё `@click="router.replace(...)"` — их стоит перевести на `:to`; в Storybook нет роутера, поэтому story для кнопки-ссылки пока не добавить.

### [P1] Компоненты M0.B: barrel и stories `done` (2026-07-21)
Четыре компонента (`OtpInput`, `PasswordInput`, `PhoneInput`, `Tabs`) были написаны и покрыты тестами, но не экспортированы из `@/shared/ui/base` — то есть недостижимы штатным импортом. Заодно у них не было stories, хотя это acceptance-критерий Phase 0.

Сделано:
- Компоненты выведены в barrel. `input`, `label`, `popover` осознанно оставлены внутренними: наружу идут `TextField`/`PasswordInput`/`PhoneInput` (они добавляют связь label↔input, aria-описание ошибки и интеграцию с `Form`) и `Menu`/`CommandPalette`. Обоснование — в комментарии к barrel.
- Инвариант закреплён тестом [public-api.test.ts](src/shared/ui/base/public-api.test.ts): слайс с `index.ts` обязан быть либо экспортирован, либо явно в списке внутренних. Список внутренних тоже проверяется — на исчезнувшие слайсы.
- 18 stories для четырёх компонентов, включая шкалу надёжности пароля и целевой сценарий вкладок «E-mail / Телефон» из auth-suite.
- `npm run build-storybook` добавлен в CI: обычные тесты stories не трогают, без сборки они тихо ломаются.

Проверено в браузере на статической сборке Storybook: 12/12, включая поведенческую проверку — посимвольный набор в `PhoneInput` даёт корректный `+79991234567`.
...
**Первый прогон на GitHub упал на шаге Install** — и нашёл то, чего локально было не увидеть. `@vee-validate/zod` требует peer `zod@^3`, а в проекте `zod@4`: локальный `node_modules` уцелел с прежней установки, но `npm ci` на чистой машине падает с `ERESOLVE`. Пакет при этом **не использовался вовсе** — от него отказались ещё в ADR-0010 (`toTypedSchema` крашится на Zod 4), но из зависимостей не убрали. Удалён.

Мораль в пользу самого CI: сборка на чистой машине проверяет то, что локальная разработка структурно проверить не может.

Не покрыто CI: E2E (их нет), `npm audit`.
